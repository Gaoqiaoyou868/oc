 ...
 *  ║  步进电机: P1.0~P1.3 → MX1508(IN1~IN4) → MOTOR-      ║
 *  ║            STEPPER (双极性 8 拍驱动)                   ║
 *  ║  按键KEY1: P3.1 (启动/停止)                             ║
 *  ║  按键KEY2: P3.0 (风速 +1)                               ║
 *  ║  按键KEY3: P3.2 (风速 -1)                               ║
 *  ║  按键KEY4: P3.3 (短按=切换模式, 长按2秒=设置/取消定时)  ║
 *  ║  LED(D1) : P2.0 (电源指示灯, 共阳:0=亮)               ║
 *  ║  LED(D2) : P2.1 (模式指示灯)                           ║
 *  ║  蜂鸣器  : P2.5 (操作提示音)                            ║
 *  ╚══════════════════════════════════════════════════════════╝
 *
 *  ╔══════════════════════════════════════════════════════════╗
 *  ║  Proteus 仿真接线说明                                    ║
 *  ║  - LCD1602: LM016L 模块, D0~D7 接 P0                  ║
 *  ║  - 步进电机: MOTOR-STEPPER, 通过 MX1508 驱动           ║
 *  ║             P1.0~P1.3 → MX1508.IN1~IN4                ║
 *  ║             MX1508.OUT1~OUT4 → MOTOR-STEPPER 4 根线   ║
 *  ║  - 按键: 4个 BUTTON 接 P3.0~P3.3, 对地连接,            ║
 *  ║          外接 10KΩ 上拉电阻到 VCC                      ║
 *  ║  - LED: 2个 LED 接 P2.0 和 P2.1, 串 220Ω 限流电阻     ║
 *  ║  - 蜂鸣器: BUZZER 接 P2.5, 另一脚接地                   ║
 *  ║  - 晶振: 11.0592MHz + 两个 30pF 电容接地               ║
 *  ║  - 复位: 10KΩ 上拉 + 10uF 电容接地 + 按键到 VCC       ║
 *  ╚══════════════════════════════════════════════════════════╝
 *
 *  功能说明:
...
/* ======================== 步进电机引脚 (MX1508 驱动) ======================== */
/*
 * MX1508 双 H 桥驱动步进电机 (双极性)
 * P1.0/P1.1 → MX1508 IN1/IN2 → OUT1/OUT2 → 电机线圈 1
 * P1.2/P1.3 → MX1508 IN3/IN4 → OUT3/OUT4 → 电机线圈 2
 *
 * MX1508 真值表:
 *   IN1=1,IN2=0 → OUT1=H,OUT2=L → 线圈 1 正转
 *   IN1=0,IN2=1 → OUT1=L,OUT2=H → 线圈 1 反转
 *   IN1=0,IN2=0 → OUT1=L,OUT2=L → 线圈 1 停转(释放)
 *   IN1=1,IN2=1 → OUT1=H,OUT2=H → 线圈 1 刹车
 *
 * 28BYJ-48 转双极性接法:
 *   橙(A)+黄(B) → 线圈 1 (common VCC 悬空)
 *   粉(C)+蓝(D) → 线圈 2 (common VCC 悬空)
 */
sbit IN1_A = P1^0;
sbit IN2_B = P1^1;
sbit IN3_C = P1^2;
 ...
 * 每个按键需外接 10KΩ 上拉电阻到 VCC
 *
 * 在 Proteus 中: 直接使用 BUTTON 组件,
 * 一端接单片机引脚, 另一端接地即可
 */
sbit KEY1 = P3^1;    // 启动 / 停止
sbit KEY2 = P3^0;    // 风速 +1
sbit KEY3 = P3^2;    // 风速 -1
sbit KEY4 = P3^3;    // 短按=切换模式, 长按2秒=设置/取消定时
 ...
 *
 * 蜂鸣器: P2.5 (无源蜂鸣器, 需 PWM, 与 LCD_RW 共用)
 */
sbit LED_D1 = P2^0;
sbit LED_D2 = P2^1;
sbit BEEP   = P2^5;
...
/* --- 步进电机 8 拍序列 (MX1508 双极性半拍) --- */
/*
 * MX1508 双 H 桥驱动, 8 拍半步方式
 * 每拍切换两个 H 桥的输入状态
 * 按此序列循环 = 顺时针旋转
 *
 * 写入方式: P1 = (P1 & 0xF0) | phase_table[idx]
 * 因此 phase_table 只使用低 4 位:
 *   bit0=P1.0=IN1, bit1=P1.1=IN2  → 控制线圈 1 (OUT1/OUT2)
 *   bit2=P1.2=IN3, bit3=P1.3=IN4  → 控制线圈 2 (OUT3/OUT4)
 *
 * 电机参数: 28BYJ-48, 减速比 1:64
 *          双极性 8 拍: 4096 步/转
 *          步距角: 5.625° / 64 = 0.08789°
 */
u8 code phase_table[8] = {
    0x0A,  // 1010: 线圈1正转, 线圈2正转
    0x08,  // 1000: 线圈1正转, 线圈2释放
    0x09,  // 1001: 线圈1正转, 线圈2反转
    0x01,  // 0001: 线圈1释放, 线圈2反转
    0x05,  // 0101: 线圈1反转, 线圈2反转
    0x04,  // 0100: 线圈1反转, 线圈2释放
    0x06,  // 0110: 线圈1反转, 线圈2正转
    0x02   // 0010: 线圈1释放, 线圈2正转
};

/* --- 风速档位对应的步进延时 (ms) --- */
/*
 * speed_delay[i] 表示第 (i+1) 档的步进间隔
 * 值越小 = 转速越快
 *
 * 新速度表 (大幅拉开差距, 肉眼可辨):
 *   1档(微风): 30ms/步 → 约 123 秒/转  ← 非常慢, 叶片缓缓转动
 *   2档      : 20ms/步 → 约  82 秒/转
 *   3档(中速): 12ms/步 → 约  49 秒/转
 *   4档      :  6ms/步 → 约  25 秒/转
 *   5档(强风):  2ms/步 → 约   8 秒/转  ← 明显快, 风力强劲
 *
...
/*******************************************************************************
 * 函数名  : Motor_StepCW
 * 功能    : 步进电机顺时针走一步 (MX1508 双极性 8 拍)
 * 参数    : 无
 * 返回值  : 无
 * 说明    : 从 phase_table 中取当前相位, 写入 P1 低 4 位
 *           用 (P1 & 0xF0) | phase_table[idx] 保持高 4 位不变
 *
 *           8 拍顺序 (MX1508):
 *           线圈1正+线圈2正 → 线圈1正 → 线圈1正+线圈2反 → 线圈2反 →
 *           线圈1反+线圈2反 → 线圈1反 → 线圈1反+线圈2正 → 线圈2正 → ...
 *           每拍旋转 5.625° / 64 = 0.088°
 *
 *           ★如电机转向不对, 交换 MX1508 任意一组输出线即可
 *******************************************************************************/
void Motor_StepCW(void)
{
    phase_idx++;
 ...
 * 参数    : 无
 * 返回值  : 无
 * 说明    : P1 低 4 位清零, 电机停转, 处于无保持力矩的自由状态
 *******************************************************************************/
void Motor_Stop(void)
{
    P1 &= 0xF0;                  // 低 4 位清零
...
/*******************************************************************************
 * 函数名  : Beep_Tick
 * 功能    : 产生短促提示音 (~1KHz, 持续约 50ms)
 * 参数    : 无
 * 返回值  : 无
 * 说明    : 软件方式翻转 P2.5 产生方波
 *           50 次翻转 × 0.5ms ≈ 25ms 高 + 25ms 低 ≈ 50ms 时长
 *           频率 ≈ 1KHz
 *
 *           ★警告: 发声期间不能操作 LCD!
 *           因为 P2.5 = LCD_RW, 方波会使 LCD 误入读模式
 *           此函数应在按键处理之后、LCD 刷新之前调用
 *******************************************************************************/
void Beep_Tick(void)
{
    u8 i;
    for (i = 0; i < 50; i++) {
        BEEP = !BEEP;
        Delay_10us(50);          // 约 0.5ms 半周期
    }
    BEEP = 0;                    // 释放 P2.5 (RW=0 写模式)
}
...
void main(void)
{
    ...
    P1 = 0xF0;                   // P1 高 4 位=1, 低 4 位=0 (电机未驱动)
    P2 = 0xFF;                   // P2 全高 (LED灭, LCD控制脚就绪)
    P3 = 0xFF;                   // P3 全高 (按键输入模式)

    /* ===== ② 变量初始化 ===== */
    fan_state        = FAN_OFF;
    fan_mode         = MODE_NORMAL;
    speed_level      = 3;        // 默认 3 档 (中速)
    effective_speed  = 2;        // 索引 2 → speed_delay[2]=4ms
    phase_idx        = 0;
    sys_tick         = 0;
    natural_tick     = 0;
    sleep_tick       = 0;
    ...
    while (1) {
        ...
        if (fan_state == FAN_ON) {
            /*
             * 使用阻塞式延时控制转速:
             * 每走一步后执行 Delay_ms( speed_delay[effective_speed] )
             * 延时越长 → 转速越慢 → 风力越小
             *
             * ★改用阻塞式的原因:
             *   之前的计数器方式依赖主循环迭代次数 (=微秒级),
             *   导致 1 档和 5 档实际上转速一样, 看不出差异.
             *   阻塞式 Delay_ms 精确控制每步间隔, 差异肉眼可见:
             *
             *   1档(微风): 30ms/步 → 叶片缓慢转动
             *   5档(强风):  2ms/步 → 叶片飞速旋转
             *
