 ...
 *  ║            IN4_D=P1.3  (接 MOTOR-STEPPER 的 4 根线)    ║
 *  ║  按键KEY1: P3.1 (启动/停止)                             ║
 *  ║  按键KEY2: P3.0 (风速 +1)                               ║
 *  ║  按键KEY3: P3.2 (风速 -1)                               ║
 ...
 *  ║  - 步进电机: MOTOR-STEPPER, 4根控制线接 P1.0~P1.3      ║
 *  ║             (或通过 ULN2003 驱动芯片连接)               ║
...
/* ======================== 步进电机引脚 ======================== */
/*
 * 28BYJ-48 步进电机, 4 相, 通过 ULN2003 驱动
 * 在 Proteus 中使用 MOTOR-STEPPER 组件
 * 连接: 4 根控制线 → P1.0~P1.3
 *       电机公共端 → +5V
 */
sbit IN1_A = P1^0;
...
/* --- 步进电机 8 拍序列 --- */
/*
 * 8 拍半步驱动, 每拍给 1~2 个线圈通电
 * 按此序列循环 = 顺时针旋转
 *
 * 电机参数: 28BYJ-48, 减速比 1:64
 *          8 拍模式: 4096 步/转
 *          步距角: 5.625° / 64 = 0.08789° (8 拍)
 */
 ...
 *
 *           8 拍顺序: A→AB→B→BC→C→CD→D→DA→A...
 *           每拍旋转 5.625° / 64 = 0.088° (考虑到减速比)
 *******************************************************************************/
void Motor_StepCW(void)
{
    phase_idx++;
