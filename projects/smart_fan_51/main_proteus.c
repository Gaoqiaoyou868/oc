 ...
 *  ║  直流电机: P1.0/P1.1 → MX1508(IN1/IN2) → MOTOR      ║
 *  ║            (软件 PWM 调速)                            ║
 *  ║  按键KEY1: P3.1 (启动/停止)                             ║
 *  ║  按键KEY2: P3.0 (风速 +1)                               ║
 *  ║  按键KEY3: P3.2 (风速 -1)                               ║
 ...
 *  ║  - 直流电机: MOTOR (直流), 通过 MX1508 驱动           ║
 *  ║             P1.0 → MX1508.IN1, P1.1 → MX1508.IN2     ║
 *  ║             MX1508.OUT1/OUT2 → MOTOR 两根线           ║
...
/* ======================== 直流电机引脚 (MX1508 驱动) ======================== */
/*
 * MX1508 双 H 桥驱动直流电机 (PWM 调速)
 * P1.0/P1.1 → MX1508 IN1/IN2 → OUT1/OUT2 → 直流电机
 * P1.2 = IN3, P1.3 = IN4 悬空 (未使用)
 *
 * MX1508 真值表:
 *   IN1=1,IN2=0 → 电机正转
 *   IN1=0,IN2=1 → 电机反转
 ...
 * 值越大 → 占空比越高 → 转速越快
 *
 * 写入方式: P1 = (P1 & 0xF0) | 0x01  (IN1=1,IN2=0 → 正转)
 *          P1 = (P1 & 0xF0) | 0x00  (IN1=0,IN2=0 → 停止)
 *
...
/*******************************************************************************
 * 函数名  : Motor_Forward
 * 功能    : 直流电机正转 (MX1508 IN1=1, IN2=0)
 * 参数    : 无
 * 返回值  : 无
 * 说明    : MX1508 OUT1=H, OUT2=L → 电机正转
 *           配合外部 Delay_ms 实现 PWM 调速
 *******************************************************************************/
void Motor_Forward(void)
{
    P1 = (P1 & 0xF0) | 0x01;    // IN1=1, IN2=0
}

/*******************************************************************************
 * 函数名  : Motor_Stop
 * 功能    : 直流电机停止 (MX1508 IN1=0, IN2=0)
 * 参数    : 无
 * 返回值  : 无
 * 说明    : MX1508 OUT1=L, OUT2=L → 电机自由停车
 *******************************************************************************/
void Motor_Stop(void)
{
    P1 &= 0xF0;                  // IN1=0, IN2=0
