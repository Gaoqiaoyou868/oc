 ...
 *  ║  步进电机: P1.0~P1.3 → MX1508(IN1~IN4) → 28BYJ-48   ║
 *  ║            (双极性 8 拍驱动, JP8=ON, JP4=OFF)          ║
 *  ║  红外接收: P3.2 (外部中断0)                             ║
...
/* ======================== 步进电机引脚 (MX1508 驱动) ======================== */
/*
 * 28BYJ-48 步进电机, 通过 MX1508 双 H 桥驱动 (双极性模式)
 * 8 拍半步工作方式, 每转 4096 步 (减速比 1:64)
 *
 * 接线: P1.0/P1.1 → MX1508 IN1/IN2 → OUT1/OUT2 → 线圈 1 (橙+黄)
 *       P1.2/P1.3 → MX1508 IN3/IN4 → OUT3/OUT4 → 线圈 2 (粉+蓝)
 *       28BYJ-48 公共端 VCC (红线) 悬空
 *
 * 普中A2板注意: P1 与矩阵键盘 JP4 共用,
 * JP8=ON (接 MX1508), JP4=OFF (断开矩阵键盘)
 */
sbit IN1_A = P1^0;
sbit IN2_B = P1^1;
sbit IN3_C = P1^2;
 ...
 *
 * MX1508 真值表:
 *   IN1=1,IN2=0 → 线圈正转;  IN1=0,IN2=1 → 线圈反转
 *   IN1=0,IN2=0 → 线圈释放;  IN1=1,IN2=1 → 线圈刹车
 *
 * 每拍旋转 5.625° / 64 = 0.08789°
 */
