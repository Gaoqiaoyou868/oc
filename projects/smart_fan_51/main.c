 ...
 *  ║  直流电机: P1.0 → L298.IN1, P1.1 → L298.IN2          ║
 ...
 *   VSS → +5V (逻辑供电)
 *   SENSE_A → GND
 *   IN3/IN4/ENB/OUT3/OUT4 → 悬空 (只用 A 桥)
 *
 * 普中A2板注意: P1 与矩阵键盘 JP4 共用,
 * JP8=ON (接 L298), JP4=OFF (断开矩阵键盘)
 *
 * L298 A桥真值表 (ENA=1):
 *   IN1=1,IN2=0 → 电机正转
 *   IN1=0,IN2=1 → 电机反转
...
void main(void)
{
    ...
    P1 = 0xF0;                   // P1 高 4 位=1 (按键上拉输入), 低 4 位=0 (电机未驱动)
    ...
    effective_speed  = 2;          // 对应 pwm_on_time[2] = 12ms
    sys_tick         = 0;
    natural_tick     = 0;
    sleep_tick       = 0;
    timer_tick       = 0;
